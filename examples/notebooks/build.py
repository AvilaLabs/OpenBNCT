...
nb1 = nb([
    ...
    ("code",
     ...
     "bundle = openbnct.load_physical_dose_bundle(BENCH / 'dose-28g-1e.json')\n"
    ...
    ("md", "## Mid-plane dose map\n\n"
           "The bundle stores x-fastest linearized voxel data; reshape to "
           "the grid and slice the central z-plane."),
    ("code",
     "import matplotlib.pyplot as plt\n"
     "import numpy as np\n"
     "\n"
     "nx, ny, nz = bundle.geometry.shape\n"
     "total = np.array(bundle.physical_total.values).reshape(nx, ny, nz)\n"
     "mid = total[:, :, nz // 2]\n"
     "\n"
     "fig, ax = plt.subplots(figsize=(5, 4))\n"
     "im = ax.imshow(np.log10(mid.T + 1e-30), origin='lower', cmap='inferno')\n"
...
nb2 = nb([
    ...
    ("code",
     ...
     "\n"
     "nx, ny, nz = bundle.geometry.shape\n"
     "a = np.array(bio.biological_total.values).reshape(nx, ny, nz)[:, :, nz // 2]\n"
     "c = np.array(isoe.biological_total.values).reshape(nx, ny, nz)[:, :, nz // 2]\n"
     "\n"
     "fig, axes = plt.subplots(1, 3, figsize=(13, 4))\n"
     "for ax, vol, title in zip(axes, [a, c, c / np.maximum(a, 1e-30)],\n"
     "                          ['fixed-weights', 'G&S IsoE', 'IsoE / fixed ratio']):\n"
     "    im = ax.imshow(np.log10(vol.T + 1e-30) if 'ratio' not in title\n"
     "                   else vol.T, origin='lower', cmap='inferno')\n"
     "    ax.set_title(title)\n"
