#!/bin/bash
# Collapse the layered-head phantom materials to 28-group data.
...
openbnct sn collapse \
  ...
  --boundaries "$BOUNDS" \
  --id openbnct.layered-head-phantom.multigroup-28g.v1 \
  --component-profile "$PROFILE" \
  --note "ICRU-44 tissues; H/C/N/O/B10 from 294K HDF5, tissue nuclides from NJOY 293.6K PENDF (ENDF/B-VIII.1)" \
