#!/bin/bash
# Collapse the layered-head phantom materials to 28-group data with
# Bondarenko heterogeneous-dilution self-shielding (v3: same inputs and
# grid as v2 — collapse-v2.sh — plus --self-shielding, isolating the
# resonance-suppression correction against an identical baseline).
...
"$OPENBNCT" sn collapse \
  ...
  --boundaries "$BOUNDS" \
  --self-shielding \
  --id openbnct.layered-head-phantom.multigroup-28g.v3-shielded \
  --component-profile "$PROFILE" \
  --note "ICRU-44 tissues; H/C/N/O/B10 from 294K HDF5, tissue nuclides from NJOY 293.6K PENDF (ENDF/B-VIII.1). v3: Bondarenko heterogeneous-dilution self-shielding on (v2 inputs and grid otherwise identical — v2 remains the unshielded record)" \
  --output "$ROOT/multigroup-data-28g-v3-shielded.json"
