...
//! `interchange_from_niftis` → `import_component_dose` and pins both the
...
//! `OPENBNCT_UPDATE_CONFORMANCE=1 cargo test -p openbnct-nifti`.

use std::fs;
use std::path::{Path, PathBuf};

use openbnct_core::{DoseComponent, DoseUnit, import_component_dose};
use openbnct_nifti::{NiftiComponentSource, interchange_from_niftis};
...
#[test]
fn nifti_adapter_conformance_suite() {
    let root = suite_dir();
    ...
    for case in manifest.cases.iter().filter(|c| c.system == "nifti") {
        let sources: Vec<NiftiComponentSource> = case
            .sources
            .iter()
            .map(|s| NiftiComponentSource {
                component: component(&s.component),
                file: PathBuf::from(&s.file),
                sigma_file: s.sigma_file.as_ref().map(PathBuf::from),
            })
            .collect();
        let document = interchange_from_niftis(
            ...
            unit(&case.unit),
            &case.normalization,
            case.producer_system
                .as_deref()
                .expect("nifti cases declare producer_system"),
            case.producer_version.clone(),
            None,
        )
    ...
    assert!(count > 0, "suite carries no nifti cases");
}
