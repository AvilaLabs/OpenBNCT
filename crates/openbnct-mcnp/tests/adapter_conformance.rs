...
//! Public adapter conformance suite: replays the MCNP cases in
//! `conformance/adapters/0.1.0/manifest.json` through `parse_meshtal` →
//! `interchange_from_meshtals` → `import_component_dose` and pins both the
...
//! `OPENBNCT_UPDATE_CONFORMANCE=1 cargo test -p openbnct-mcnp`.

use std::fs;
use std::path::{Path, PathBuf};

use openbnct_core::{DoseComponent, DoseUnit, import_component_dose};
use openbnct_mcnp::{ComponentSource, interchange_from_meshtals};
...
#[test]
fn mcnp_adapter_conformance_suite() {
    let root = suite_dir();
    ...
    for case in manifest.cases.iter().filter(|c| c.system == "mcnp") {
        let sources: Vec<ComponentSource> = case
            ...
            .map(|s| ComponentSource {
                ...
                tally: s.tally.expect("mcnp sources name a tally"),
                energy_bin: s.energy_bin,
            })
            .collect();
        let document = interchange_from_meshtals(
            &sources,
            &case.case_id,
            unit(&case.unit),
            &case.normalization,
    ...
    assert!(count > 0, "suite carries no mcnp cases");
}
